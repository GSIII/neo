...
SELECT userid AS "사용자명", sum(amount) AS "총구매량" FROM buytbl GROUP BY userid;

SELECT userid AS "사용자명", sum(amount * price) AS "총구매액" FROM buytbl GROUP BY userid;

SELECT cast(avg(amount) AS number(5,3)) AS "평균 구매 수량" FROM buytbl;

SELECT cast(avg(amount) AS number(5,3)) AS "평균 구매 수량" FROM buytbl GROUP BY userid;
...
SELECT userid AS "사용자명", sum(amount * price) AS "총구매액" FROM buytbl GROUP BY userid HAVING sum(price * amount) > 1000 ORDER BY sum(price * amount);
...
DELETE FROM cubetbl WHERE amount=11;
...
SELECT prodname, color, sum(amount) AS "수량 합계" FROM CUBETBL c GROUP BY cube(color,prodname) order BY prodname, c.COLOR ;
...
SELECT concat(rpad('ㄴ', emplevel * 2 + 1, 'ㄴ'), empname) AS "직원이름", dept AS "직원부서" FROM empcte ORDER BY dept, emplevel;
...
ALTER SEQUENCE idseq2 INCREMENT BY 5;
...
CREATE SEQUENCE cycleseq START WITH 100 INCREMENT BY 100 MINVALUE 100 MAXVALUE 300 CYCLE nocache;
...
UPDATE testtbl4 SET firstname='David' WHERE empid=100;
...
AS 
	SELECT LEVEL 
AS bidid, round(dbms_random.value(1,500000), 0) AS numdata 
	FROM dual 
...
SELECT row_number() OVER (ORDER BY height desc) "키 큰 순위", username, addr, height FROM usertbl;

SELECT row_number() OVER (ORDER BY height asc) "키 작은 순위", username, addr, height FROM usertbl;

SELECT addr, row_number() OVER (PARTITION BY addr ORDER BY height DESC, username asc) "키 큰 순위", username, addr, height FROM usertbl;

SELECT DENSE_RANK() OVER (ORDER BY height DESC) "키 큰 순위", username, addr, height FROM usertbl;

SELECT RANK() OVER (ORDER BY height DESC) "키 큰 순위", username, addr, height FROM usertbl;

SELECT NTILE(3) over(ORDER BY height DESC) "반번호", username, addr, height FROM USERTBL u ;

SELECT username, addr, height AS "키", height - (LEAD(height,1,0) over(ORDER BY height desc)) AS "다음 사람과의 키 차이" FROM usertbl;

SELECT username, addr, height AS "키", height - (FIRST_VALUE(height) over(PARTITION BY addr ORDER by height desc)) AS "지역별 최대 키와 차이" FROM usertbl;

SELECT username, addr, height AS "키", (CUME_DIST() over(PARTITION BY addr ORDER by height desc)) * 100 AS "누적 인원 백분율 (%)" FROM usertbl;
