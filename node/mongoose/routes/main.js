...
app.get("/get", async function (req, res) {
  try {
    ...
    var user = await User.findOne({ userid: userid }).select("-_id"); // projection({"_id":0})와 같은 의미
    if (!user) {
...
app.post("/insert", async function (req, res) {
  try {
    ...
    if (!userid || !name || !city || !sex || !age) {
      return res.status(400).send("Userid, Name, City, Sex, Age are required");
    }
...
// select userid, name, age from users where city='Seoul' and age > 20 and age < 23
function query5(callback) {
  User.find({ city: "Seoul", age: { $gt: 20, $lt: 23 } }, { _id: 0 })
    .sort({ age: -1 })
...
// using querybuilder
// select userid, name, age from users where city='Seoul' and age > 15 and age < 30 order by age
